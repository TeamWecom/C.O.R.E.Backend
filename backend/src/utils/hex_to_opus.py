...
class srtp_ogg_opus_coder(ogg_opus_coder):
    ...
    def record_rtp_packet(self, packet,is_last=False):
        ...
        if rtp.PAYLOAD_TYPE & 0b10000000:
            ...
            rtp = RTP_HEADER(FIRST=rtp.FIRST,PAYLOAD_TYPE=rtp.PAYLOAD_TYPE&0b01111111,SEQUENCE_NUMBER=rtp.SEQUENCE_NUMBER,TIMESTAMP=rtp.TIMESTAMP,SSRC=rtp.SSRC)
    ...
    def record_rtp_file(self, infile, outfile, hexstring=False):
        ...
        if hexstring:
            with open(infile, 'r') as rtp_fd:
                ...
                for xline in rtp_fd:
                    xline = xline.strip()
                    packet = [xline[idx:idx+2] for idx in range(0,len(xline),+2)]
                    packet_counter += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hexstring'
        , action='store_true'
        , help='Text output of pcap, hint `tshark -x -r file.pcap -Y "rtp && udp.srcport == myport" -T fields -e data > strm_from_myport.txt`\n'
