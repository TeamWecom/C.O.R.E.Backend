...
import { getMicrosoftTokens, loadMicrosoftTokens } from '../managers/microsoftCalendarManager.js';
...
// Rota para confirmar Google OAuth
router.get('/microsoft-oauth-callback', async (req, res) => {
    try {
        const code = req.query.code; // Obtém o valor de 'code' da URL
        if (!code) {
            return res.status(400).send('Código de autorização não encontrado.');
        }
        await getMicrosoftTokens(code)
        const status = await loadMicrosoftTokens()
        broadcast({ api: "admin", mt: "RequestMicrosoftOAuthStatusResult", result: status })
        res.status(200).send(`<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Microsoft OAuth - CORE</title>
      <style>
            body {
                font-family: Arial, sans-serif;
                background-color: #f4f4f4;
